class SalesBsController < ApplicationController
     def index
        if params[:start_date] && params[:end_date]
            ...
            sales_data = SalesB.where(date: start_date..end_date)
    ...
    def create
       products=Product.where(shop: "gateB", name: params[:product])
        product= products.first
         ...
         if product.nil?
            ...
            sale=SalesB.create(product:params[:product],price:params[:price],date:params[:date],employee:params[:employee],quantity:params[:quantity], status:params[:status])
                if sale.save
    ...
    def update
        ...
        product= Product.find_by(name: params[:product], shop: "gateB")
        if product.nil?
            deletedproduct=Product.create(name:params[:product],shop:"gateB",quantity: original_quantity,price: sale.price , category_id: 4 )
